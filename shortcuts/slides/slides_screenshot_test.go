...
func TestSlidesScreenshotRenderContentWritesFile(t *testing.T) {
	...
	if body.XMLPresentation.PresentationID != "pres_abc" {
		t.Fatalf("presentation_id = %q, want pres_abc", body.XMLPresentation.PresentationID)
	}
	if body.XMLPresentation.RevisionID != 7 {
		t.Fatalf("revision_id = %d, want 7", body.XMLPresentation.RevisionID)
	}
	if body.XMLPresentation.Content != content {
		t.Fatalf("content = %q, want input XML", body.XMLPresentation.Content)
	}
