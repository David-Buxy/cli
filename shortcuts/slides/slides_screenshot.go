...
var SlidesScreenshot = common.Shortcut{
	...
	Flags: []common.Flag{
		{Name: "presentation", Desc: "xml_presentation_id, slides URL, or wiki URL that resolves to slides; optional with --content"},
		...
		{Name: "revision-id", Type: "int", Default: "-1", Desc: "presentation revision (-1 = latest)"},
	},
	Validate: func(ctx context.Context, runtime *common.RuntimeContext) error {
		...
		if renderMode {
			...
			if presentation := strings.TrimSpace(runtime.Str("presentation")); presentation != "" {
				ref, err := parsePresentationRef(presentation)
				if err != nil {
					return err
				}
				if ref.Kind == "wiki" {
	...
	Execute: func(ctx context.Context, runtime *common.RuntimeContext) error {
		...
		query.Set("revision_id", strconv.Itoa(runtime.Int("revision-id")))
		body := map[string]interface{}{}
		if len(slideIDs) > 0 {
			body["slide_ids"] = slideIDs
		}
...
func dryRunRenderScreenshot(runtime *common.RuntimeContext) *common.DryRunAPI {
	...
	xmlPresentation := map[string]interface{}{
		"revision_id": runtime.Int("revision-id"),
	...
	if presentation := strings.TrimSpace(runtime.Str("presentation")); presentation != "" {
		...
		if ref.Kind == "wiki" {
			presentationID = "<resolved_slides_token>"
			dry.Desc("2-step orchestration: resolve wiki → render XML screenshot").
				GET("/open-apis/wiki/v2/spaces/get_node").
				Desc("[1] Resolve wiki node to slides presentation").
				Params(map[string]interface{}{"token": ref.Token})
		}
		xmlPresentation["presentation_id"] = presentationID
	}
	dry.POST("/open-apis/slides_ai/v1/slide_image/render").
		Body(map[string]interface{}{"xml_presentation": xmlPresentation})
...
func executeRenderScreenshot(runtime *common.RuntimeContext) error {
	...
	data, err := runtime.DoAPIJSONWithLogID("POST", "/open-apis/slides_ai/v1/slide_image/render", larkcore.QueryParams{}, map[string]interface{}{
		"xml_presentation": xmlPresentation,
	})
	if err != nil {
		return err
	}
