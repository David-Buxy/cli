...
| `--todo-list <json>` | 二选一 | 待办数组 JSON（请求体字段 `todo_items`）；支持 `@file` / `@-`（stdin）；与 `--todo`/`--is-done` 互斥 |
...
| CLI / JSON | API 字段 | 说明 |
|------------|---------|------|
| `--todo-list` 数组 | `todo_items` | 待办列表（必填，至少一条） |
...
| 错误现象 | 根本原因 | 解决方案 |
|---------|---------|---------|
| 参数无效 | `minute_token` 缺失，或 `todo_items` 为空 | 检查 token 与 JSON 数组 |
