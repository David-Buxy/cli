...
**CRITICAL — 需要语义图标时，MUST 先用 [`scripts/iconpark_tool.py`](scripts/iconpark_tool.py) 的 `search` 或 `resolve` 获取 `iconType`；不要凭记忆拼写 IconPark 路径，不要阅读全文 `iconpark-index.json`。如果没有合适结果，用 XML-native shape/line/text 兜底。**
