...
| 参数 | 必需 | 说明 |
|------|------|------|
| `--presentation` | list 模式必需；render 模式可选 | `xml_presentation_id`、`/slides/` URL，或解析后为 slides 的 `/wiki/` URL。传 `--content` 时会作为 render 的 `presentation_id` |
...
| `--revision-id` | 否 | 演示文稿版本号，默认 `-1` 表示最新版 |
...
如果不知道页面 ID，先回读演示文稿 XML 并提取 `<slide id="...">`：
...
3. 本地 XML 预览时，传 `--content @file` 或 `--content -`，内容应为单个 `<slide>` XML 片段；此时不要传 `--slide-id` / `--slide-number`。
